...
-- Fixed IS_Concurrent 11-28-2023 Team
-- Various changes 12/7/2023 Gabriel/Team
...
(
    SELECT DISTINCT
        sgbstdn_pidm                    c_sgbstdn_pidm,
        MAX(sgbstdn_term_code_eff)
        OVER(PARTITION BY sgbstdn_pidm) c_sgbstdn_term_code_eff
    FROM
        sgbstdn
), w_sfrstcr AS (
    SELECT DISTINCT
...
(
    ...
    FROM
        stvterm
    WHERE
        stvterm_code = 202420       --:main_sql_queryterms.stvterm_code
)
SELECT
    --to_char(sysdate, 'YYYYMMDD') || to_char(rownum, 'fm909')                AS Transmittal_NO,
    '' AS Transmittal_NO,
    debtors_last_name,
    last_attend_term,
    street_line1,
    ...
    is_employee,
    is_concurrent,
    payment_plan_code_10,
    payment_plan_code_71
FROM
(
SELECT DISTINCT
    CASE
    ...
    CASE
        WHEN
            w_spraddr_pr.spraddr_street_line2 IS NOT NULL
        THEN
    (
    nvl2(w_spraddr_pr.spraddr_pidm,
         ...
         nvl2(w_spraddr_ma.spraddr_pidm,
              ...
              upper(replace(w_spraddr_bd.spraddr_street_line1, '.', ''))))
    )
        || ', ' ||
    (
    nvl2(w_spraddr_pr.spraddr_pidm,
         ...
         nvl2(w_spraddr_ma.spraddr_pidm,
              ...
              upper(replace(w_spraddr_bd.spraddr_street_line2, '.', ''))))
    )
        ELSE
    (
    nvl2(w_spraddr_pr.spraddr_pidm,
...
--    END bd_exists,
    ''                                                                     AS date_of_debt,
    spbpers_ssn                                                            AS ssn,
    w_tbraccd.tbraccd_balance                                              AS principal,
    w_tbraccd.tbraccd_balance + 10.95                                      AS amount_due,
    ...
    CASE
        WHEN
            c_sgbstdn_term_code_eff = w_saradap_term_code_entry
            AND sgbstdn_styp_code = 'L'
    ...
    CASE
        WHEN
            ...
            FROM
                sprhold
            WHERE
                sprhold_pidm = spriden_pidm
                AND sprhold_hldd_code = 10
                AND sprhold_to_date > sysdate)
        THEN
            'Y'
        ELSE
            'N'
        END                                                                AS payment_plan_code_10,
    CASE
        WHEN
            exists(
            SELECT
            ...
            WHERE
                sprhold_pidm = spriden_pidm
                AND sprhold_hldd_code = 71
                AND sprhold_to_date > sysdate)
        THEN
            'Y'
        ELSE
            'N'
        END                                                                AS payment_plan_code_71
--    CASE
--        WHEN
...
FROM
    ...
    LEFT JOIN c_sgbstdn ON c_sgbstdn_pidm = spriden_pidm
    ...
    LEFT JOIN w_saradap ON w_saradap_pidm = spriden_pidm
    LEFT JOIN saradap ON saradap_pidm = w_saradap_pidm
                    AND saradap_term_code_entry = w_saradap_term_code_entry
    LEFT JOIN sgbstdn on c_sgbstdn_pidm = sgbstdn_pidm
                AND sgbstdn_term_code_eff = c_sgbstdn_term_code_eff
    LEFT JOIN sprhold ON sprhold_pidm = spriden_pidm
WHERE
    w_sfrstcr_max_term_code < :main_sql_queryterms.stvterm_code        --:main_sql_queryterms.stvterm_code
    AND :main_btn_go != 0
ORDER BY
