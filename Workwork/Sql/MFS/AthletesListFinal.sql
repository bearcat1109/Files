...
--        OVER(PARTITION BY sgrsprt_pidm) w_sgrsprt_term_code,
    ...
    FROM
        sgrsprt
    WHERE
        sgrsprt_term_code = :main_sql_queryterms.stvterm_code
), w_spriden AS 
(
    SELECT DISTINCT 
...
(
    SELECT DISTINCT
        shrtgpa_pidm            t_shrtgpa_pidm,
        shrtgpa_hours_earned    t_shrtgpa_hours_earned
    FROM 
        shrtgpa
    WHERE 
        shrtgpa_term_code = 
...
SELECT DISTINCT
    ...
    t_shrtgpa_hours_earned                    term_hours,
...
FROM
    ...
    JOIN sorlcur ON sorlcur_pidm = w_sgrsprt_pidm
        AND sorlcur_seqno = 
            (
                SELECT 
                ...
                FROM
                    sorlcur s1
                WHERE
                    s1.sorlcur_pidm = sgrsprt_pidm
            )
    JOIN sgbstdn ON sgbstdn_pidm = w_sgrsprt_pidm
    ...
    LEFT JOIN t_shrtgpa ON t_shrtgpa_pidm = sgrsprt_pidm
    ...
    LEFT JOIN stvmajr maj_stvmajr ON maj_stvmajr.stvmajr_code = sgbstdn_majr_code_1
