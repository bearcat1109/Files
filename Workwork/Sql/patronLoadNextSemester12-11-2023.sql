...
SELECT DISTINCT
    ...
    CASE
        WHEN
            sysdate < (SELECT w_stvterm_start_date FROM w_stvterm)
        THEN
            to_char((SELECT w_stvterm_start_date FROM w_stvterm), 'YYYY-MM-DD"T"HH:MM:SS')
        WHEN 
            sysdate < (SELECT w_stvterm_start_date FROM w_stvterm)
        THEN
    ...
    --to_char(sysdate, 'YYYY-MM-DD"T"HH:MM:SS') AS oclcExpirationDate,
    CASE
        WHEN
            sysdate < (SELECT w_stvterm_start_date FROM w_stvterm)
        THEN
