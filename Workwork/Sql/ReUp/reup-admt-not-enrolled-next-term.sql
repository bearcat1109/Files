-- ReUp Admitted but not Enrolled - Gabriel Berres 6-12-2024
...
WHERE
    ...
    AND (w_sfrstcr_credit_hr = 0 OR w_sfrstcr_credit_hr IS NULL)
ORDER BY
    spriden_id ASC,
...
SELECT
    ...
    CASE 
        WHEN
            sarappd_apdc_code IN ('GC', 'GD', 'GP', 'GT', 'DC', 'GM', 'AA', 
                                  'AD', 'AP')
        THEN
            'Decided Yes'
        ELSE
...
FROM
    goradid
    JOIN sarappd on sarappd_pidm = goradid_pidm
        AND sarappd_term_code_entry = '202510'
    JOIN sfrstcr ON sfrstcr_pidm = goradid_pidm
        AND sfrstcr_term_code = '202510'
WHERE
