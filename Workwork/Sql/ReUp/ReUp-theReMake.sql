...
--        )
    ...
    WHERE
        ...
        AND to_number(sfrstcr_term_code) >= 202510
    GROUP BY
        sfrstcr_pidm,
        sfrstcr_term_code
),  w_shrttrm AS (
...
FROM
    ...
    -- Student/Major
    LEFT JOIN w_sgbstdn ON w_sgbstdn_pidm = sfbetrm_pidm
...
--        AND sfbetrm_term_code = w_sfrstcr_term_code
    ...
    LEFT JOIN w_saradap_appl ON w_saradap_appl_pidm = sfbetrm_pidm
    LEFT JOIN w_saradap_apst ON w_saradap_apst_pidm = sfbetrm_pidm
    --JOIN stvapst ON stvapst_code = saradap_apst_code
    LEFT JOIN w_sarappd ON w_sarappd_pidm = sfbetrm_pidm
